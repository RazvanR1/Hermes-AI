def chat(message: str):
    ...
    if "proxmox" in q and ("update" in q or "actualiz" in q or "sigur" in q):
        ...
        if d["allowed"]:
            answer = (
                ...
                f"Riscul este {d['risk']}. Totuși verifică recomandările active înainte de execuție."
            )
        else:
            answer = (
        ...
        if data["recommendations"]:
            ...
            answer += f"Recomandarea principală este pentru {top['provider']}: {top['recommendation']}"
        else:
            answer += "Nu există recomandări importante acum."

    elif "docker" in q:
        intent = "provider_docker"
        docker = next((p for p in data["providers"] if p["name"].lower() == "docker"), None)
        if docker:
            answer = f"Docker este {docker['status']}: {docker['primary']}, {docker['secondary']}."
        else:
    ...
    return {
        ...
        "confidence": 0.9,
        "answer": answer,
        "context_used": ["dashboard", "recommendations", "tasks", "providers"],
        "actions": actions,
    }

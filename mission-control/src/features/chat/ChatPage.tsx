...
function missionSummary(mission: HermesMission): string {
  ...
  if (status === "completed") {
    return `Am terminat. Misiunea „${mission.goal}” s-a încheiat cu succes, iar rezultatul a fost verificat.`;
  }
...
export default function ChatPage() {
  ...
  const [mission, setMission] =
    useState<HermesMission | null>(null);
  const [busyMissionId, setBusyMissionId] =
  ...
  const {
    ...
    onUpdate: (freshMission) => {
      ...
      const currentStatus =
        freshMission.status.toLowerCase();
  ...
  const liveLabel = useMemo(() => {
    if (pollingError) {
      return "Reconectare live...";
    }

    if (polling) {
      return "Live tracking activ";
    }
  ...
  async function submitMessage(
    ...
    try {
      ...
      previousStatusRef.current =
        response.mission.status.toLowerCase();
      ...
      addAssistantMessage(
        `Nu am putut crea misiunea: ${messageText}`,
      );
  ...
  async function approveAndRun(
    ...
    if (stepIds.length === 0) {
      setError(
        "Nu există pași care așteaptă aprobare.",
      );
      return;
    }
    ...
    setError(null);
    setBusyMissionId(
      targetMission.mission_id,
    );

    addAssistantMessage(
      "Aprobarea a fost primită. Pornesc execuția și urmăresc verificarea în timp real.",
    );
    ...
    try {
      ...
      addAssistantMessage(
        `Execuția nu a putut porni: ${messageText}`,
      );
    }
  }
  ...
  async function reject(
    ...
    try {
      const results = await rejectMission(
        targetMission.mission_id,
        stepIds,
      );
      ...
      addAssistantMessage(
        `Am respins misiunea „${targetMission.goal}”. Nu a fost executată.`,
      );
    } catch (requestError) {
      const messageText =
        getApiErrorMessage(requestError);
      ...
      addAssistantMessage(
        `Nu am putut respinge misiunea: ${messageText}`,
      );
    } finally {
      setBusyMissionId(null);
    }
  ...
  return (
    <section className="relative overflow-hidden rounded-[32px] border border-white/8 bg-[#07101b]/90 shadow-[0_30px_100px_rgba(0,0,0,0.35)]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Întreabă, aprobă și urmărește
              execuția în același loc.
            </p>
          </div>
        </div>
      ...
      <div className="relative mx-auto max-w-6xl px-5 py-6 sm:px-7 lg:px-10">
        {messages.length === 0 ? (
          <div className="pb-7 pt-2 text-center">
            ...
            <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-slate-500">
              Spune-mi ce vrei să verific sau
              să execut. Operațiile riscante
              rămân blocate până la aprobarea
              ta.
            </p>
